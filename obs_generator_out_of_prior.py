import numpy as np
from wave_GP import wave
from wave_GP import wave_speed_matern
...
def save_smooth_obs_out_of_prior():
    N_x=512
    N_KL=64
    ...
    for freq in fmT:
        ...
        ax.set_xlabel('x')
        ax.set_ylabel('time')
        plt.savefig('./obs/obs_smooth_out_of_prior/freq_{}.pdf'.format(freq), format='pdf', dpi=300)
    ...
    np.savez('./obs/obs_smooth_out_of_prior/obs.npz', N_KL=N_KL, N_x=N_x, obs_true=obs_true, noise_vec=noise_vec, param_true=p )
