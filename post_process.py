...
from wave_GP import wave_speed_GP
...
def post_process_AIES():
    ...
    #with open('./stat/stat_smooth_pCN.pickle', 'rb') as handle:
    #    stat_data = pickle.load(handle)
    #stat_data = np.load('./stat/stat_smooth_pCN.npz')
    ...
    f, ax = plt.subplots(1)
    for i in range(5):
        ax.plot(samples[:,i], label=r'$\beta_{}$'.format(i+1))
...
#    ax.set_xticklabels(['0','10K','20K','30K'])
    ...
    plt.tight_layout()
    plt.savefig('./plots/trace_AIES.pdf',dpi=300)
    ...
    hdi_lower, hdi_higher = hpd_interval_numpy(samples, mass=0.99)
    ...
    # Convert interval endpoints -> asymmetric error lengths
    intervals = np.vstack([
        means[:25] - hdi_lower[:25],    # lower error
        hdi_higher[:25] - means[:25]    # upper error
    ])

    # Plot (same parameters as before)
    ax.errorbar(x, means[:25], yerr=intervals, fmt='o', elinewidth=3, label=r'99% HPD', alpha=0.7)
    ax.scatter(x, p_true[:25], label=r'true $\beta_j$', color='orange')
    ...
    ax.set_ylim(-3,3)
    plt.tight_layout()
    plt.savefig('./plots/hdi_AIES.pdf', dpi=300)
    ...
    plt.tight_layout()
    plt.savefig('./plots/posterior_samples_AIES.pdf',dpi=300)
    ...
    plt.tight_layout()
    plt.savefig('./plots/mean_uq_AIES.pdf',dpi=300)
    plt.show()
...
def post_process_gibbs():
    ...
    with open('./stat/stat_Gibbs_smooth.pickle', 'rb') as handle:
        stat_data = pickle.load(handle)

    samples_p_0 = stat_data['p']
    samples_s_0 = stat_data['s']

    with open('./stat/stat_Gibbs_smooth_1.pickle', 'rb') as handle:
        stat_data = pickle.load(handle)
    ...
    samples_p = np.concatenate( [samples_p_0,samples_p_1], axis=0 )
    samples_s = np.concatenate( [samples_s_0,samples_s_1], axis=0 )
    ...
    ax.set_xticklabels(['0','0.25K','0.5K','0.75K','1K'])
    ax.legend(fontsize = 16)
    ax.grid()
    plt.tight_layout()
    ...
    hdi_lower, hdi_higher = hpd_interval_numpy(samples_p, mass=0.99)
    ...
    #intervals = intervals - means
    #intervals[0,:] = -intervals[0,:]
    interval_size = hdi_higher[:25]-hdi_lower[:25]

    ax.errorbar(np.array(list(range(25))), means, yerr=interval_size, fmt='o', elinewidth=3, label=r'95% HPD', alpha=0.7)
    ax.scatter(np.array(list(range(25))), p_true[:25], label=r'true $\beta_j$', color='orange')
    ax.set_xlabel(r'index $j$',fontsize = 18)
    ax.set_ylabel(r'$\beta_j$', fontsize = 18)
    ax.set_xticks([0,4,9,14,19,24])
...
def post_process_out_of_prior():
    obs_data = np.load('./obs/obs_smooth_out_of_prior/obs.npz')
    print(obs_data.files)
    p_true = obs_data['param_true']
    N_x = obs_data['N_x']
    N_KL = obs_data['N_KL']
    field_GP = wave_speed_GP(N_x,N_KL)

    #f,ax = plt.subplots(1, figsize=[6,3])
    #field_GP.plot_curve(p_true,ax, label='true seabed', color='blue')
    #plt.show()
    #exit()
    ...
    f,ax = plt.subplots(1, figsize=[6,3])
    x = np.linspace(-3,3,64, endpoint=False)
    ax.plot(x,p_true, 'yo', label='original noise')

    interp = field_GP.field.interpolated(p_true)
    ax.plot(field_GP.x_grid,interp, 'g', label='interpolated noise')
    field_GP.plot_curve(p_true,ax, label='true seabed', color='blue')
    #field.plot_curve(samples_mean,ax, label='mean seabed', color='orange')
    #field.plot_uq(samples_p, ax, label='99% CI')
    ax.set_xlabel('x (km)',fontsize = 12)
    ...
    ax.set_ylim(-2,2)
    plt.tight_layout()
    plt.savefig('./plots/custom_signal.pdf',dpi=300)
    ...
    with open('./stat/stat_Gibbs_smooth_out_of_prior.pickle', 'rb') as handle:
        stat_data = pickle.load(handle)

    samples_p = stat_data['p']
    samples_s = stat_data['s']
    ...
    ax.set_xticklabels(['0','0.125K','0.25K','0.375K','0.5K'])
    ax.legend(fontsize = 16)
    ax.grid()
    plt.tight_layout()
    ...
    ax.grid()
    plt.tight_layout()
    plt.savefig('./plots/custom_trace_Gibbs_s.pdf',dpi=300)
    ...
    ax.set_ylim(-3,3)
    plt.tight_layout()
    plt.savefig('./plots/custom_hdi_Gibbs.pdf',dpi=300)
    ...
    ax.legend(fontsize = 12)
    plt.tight_layout()
    plt.savefig('./plots/custom_mean_uq_Gibbs.pdf',dpi=300)
